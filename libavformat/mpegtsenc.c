...
static int mpegts_init(AVFormatContext *s)
{
    ...
    if (ts->mux_rate > 1) {
        ...
        ts->sdt_packet_period = 500;
        ts->pat_packet_period = 38;
                             ...
                             int64_t pts, int64_t dts, int key, int stream_id)
{
    ...
    if (ts->flags & MPEGTS_FLAG_PAT_PMT_AT_KEYFRAMES && st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO && key && !ts_st->prev_payload_key) {
        force_pat = 1;
    }
...
static const AVOption options[] = {
    ...
    { "pat_pmt_at_frames", "Reemit PAT and PMT at each video frame",
      ...
      AV_OPT_FLAG_ENCODING_PARAM, "mpegts_flags" },
    { "pat_pmt_at_keyframes", "Reemit PAT and PMT at each video keyframe",
      0, AV_OPT_TYPE_CONST, { .i64 = MPEGTS_FLAG_PAT_PMT_AT_KEYFRAMES}, 0, INT_MAX,
      AV_OPT_FLAG_ENCODING_PARAM, "mpegts_flags" },
